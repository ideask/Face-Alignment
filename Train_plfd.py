...
def train(train_loader, plfd_backbone, auxiliarynet, criterion, optimizer, cur_epoch):
    losses = AverageMeter()
...
def validate(my_val_dataloader, plfd_backbone, auxiliarynet, criterion, cur_epoch):
    plfd_backbone.eval()
    auxiliarynet.eval()
    losses = []
    with torch.no_grad():
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1):
        ...
        val_loss = validate(my_val_dataloader, plfd_backbone, auxiliarynet,
                            criterion, epoch)
...
def parse_args():
    ...
    parser.add_argument(
        '--snapshot',
        default='./CheckPoints/snapshot_plfd/',
        type=str,
        metavar='PATH')
    parser.add_argument(
        '--log_file', default="./CheckPoints/train_plfd.logs", type=str)
    parser.add_argument(
        '--tensorboard', default="./CheckPoints/tensorboard_plfd", type=str)
    # -- load snapshot
    parser.add_argument(
